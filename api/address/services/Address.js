...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Address.create(data);

    // Create relational data and return the entry.
    return Address.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Address.associations.map(a => a.alias));
    const data = _.omit(values, Address.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Address.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Address.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Address.attributes).reduce((acc, curr) => {
      switch (Address.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
